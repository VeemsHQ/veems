...
class TestUploadDetail:
    def test(
        ...
        # Check that thumbnails for the highest resolution of the two
        # Video Renditions were returned from the API.
        assert (
            len(response.json()['autogenerated_thumbnail_choices'])
            == num_thumbs_per_rendition
        )
        for item in response.json()['autogenerated_thumbnail_choices']:
            assert item['height'] == transcoder_profiles.Webm720p.height
            assert item['width'] == transcoder_profiles.Webm720p.width
            assert item['file'].startswith('http')
            assert item['id']

    def test_returns_404_if_attempting_to_get_another_users_upload(
        self,
        api_client_factory,
        channel_factory,
        upload_factory,
        ...
        api_client, api_user = api_client_factory(user=user_factory())
        channel = channel_factory(user=user)
        upload = upload_factory(channel=channel)
        upload_id = upload.id
        assert upload.channel.user != api_user
...
class TestUploadComplete:
    ...
    def test_put_with_upload_id_triggers_transcoding_process(
        ...
        exp_parts = [
            {'etag': p['etag'], 'part_number': int(p['part_number'])}
            for p in parts
        ]
        mock_upload_manager.complete.delay.assert_called_once_with(
            upload_id=upload_id, parts=exp_parts
        )
    ...
    def test_returns_400_if_body_contains_malformed_parts(
        self, api_client, upload_id
    ):
        api_client, _ = api_client
        parts = [
            {'etag': 'hello', 'part_number': 'world'},
        ]
        body = json.dumps({'parts': parts})
        url = f'/api/v1/upload/complete/{upload_id}/'
        response = api_client.put(url, body, content_type='application/json')
