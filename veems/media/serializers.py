...
class UploadSerializer(CustomModelSerializer):
    ...
    video_id = serializers.SerializerMethodField(method_name='get_video_id')
...
class VideoSerializer(CustomModelSerializer):
    ...
    def get_autogenerated_thumbnail_choices(self, instance):
        # TODO: TEST API LEVEL
        video = instance
        rendition = video.renditions.order_by('-height').first()
        if not rendition:
            # TODO: test
            return []

        profile_names = video.transcode_jobs.values_list('profile', flat=True)
        profile_classes = tuple(
            transcoder_profiles.get_profile(name) for name in profile_names
        )
        highest_res_profile_cls = sorted(
